 ...
 */
-(void)Service_CallWithPostData:(NSData *)data withMethodName:(wsMethodNames)MethodType
{
 ...
 */
-(void)sendRequestToServer:(wsMethodNames)methodName withObject:(id)Object withRefrence:(id)refrence
{
    _delegate = refrence;
    
    //JsonConvertor *jsonHandler = [JsonConvertor sharedInstance];
    ...
    switch(methodName)
    {
        case login:
        {
            ...
            NSData *data;// = [jsonHandler convertLoginObjectToJson:Object];
            [self Service_CallWithPostData:data withMethodName:login];
            break;
        }
               default:
            break;
...
- (void)didReciveResponse:(NSData *)responsedata withMethodName:(wsMethodNames)name
{
    ...
    NSDictionary *mainResult = [jsonArray objectForKey:@"result"];
    BOOL success = [[mainResult objectForKey:@"success"] boolValue];
    ...
    if(success == YES)
    {
        switch(name)
        {
            case login:
            {
               ...
               // [jsonConvertor getLoginResponseObject:jsonArray];
                respObjct = nil;
    ...
    {
        error = [[mainResult objectForKey:@"returnCode"] intValue];
    }
